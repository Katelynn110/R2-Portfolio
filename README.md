...
Now lets look at a line with having the x axis as dates. we'll use the built in economics time series for this example.
...
Now lets try error bars on the violin plot
...
What about adding jitterplots to a barplot?
...
Lets start by making basic plots. First we will define a color palette and data.
...
we can also support multiple plots to a pdf
...
In thus instance, y is the width of the diamond, so anything under 3 mm or above 20 is excluded
...
```
to make the comparison easier, we need to swap the display on the y-axis. Instead of siplaying count, we'll display density, which is the count standardized so that the area under the curve is one 
...
```
scatterplots become less useful as the size of your datasets grow because we overplot. We can fix this using the alpha aesthetic
...
# Exploratory Data Analysis (part 1 - 5; Outliers section included)
First lets load the required library
...
First lets start of with some definitions

Data - obvious - take stuff we want to visualize
...
Coordinate system - describes how the data coordinates are mapped together in rekation to the plan on the graphic
...
hist(Louisiana_Cases$cases, frew = NULL, density = NULL, breaks = 10, xlab = "Total Cases", ylab = "Frequency",
     main = "Total College Covid-19 Infections (Louisiana)")

```
Lwets run a simple histogram for the Iris data
...
```
Now lets look at the southern states cases
...
obesity <- read.csv("~/Desktop/classroom/starter/Obesity_insurance.csv")
...
```
Now lets look at correlations. The cor() command is used to determine correlations between two vectors, all of the columns of data frame, or two data frames. The cov() command, on the otherhand examines the covariance. The cor.test() command carries out a test as to the significance of the correlation
...
```
This test uses a spearman the correlation, or you can use Kendall's tau by specifying it
...
Now lets look at the tietjen=moore test. This is used for univariance datasets. The algorithm depicts the detection of the outliers in a univariate dataset
...
TietjenMoore <- function(dataSeries, k)
{
  ...
  # sort data according to the size of residual
  df = data.frame(dataSeries, r)
  dfs = df[order(df$r),]
  # create a subset of the data without the large values
  klarge = c((n-k+1):n)
...
This function helps to compute the absolute residuals and sort data according to the size of the residuals.
...
This function gelps to compute the critical valuies based on simulation data. Now lets demonstrate these functions with sample data and the obesity dataset for evaluating this algorithm

The critical region for the Tietjen-Moore test is determind by simulation. The simulation is performed by generating a standard normal random sample of the size n and computing the Tietjen Moore statistics obtained from the data is compared to this reference distribution. The values of the test statistic is between zero and one. If there are not outliers in the data, the test statistic is close to 1. If there are outliers the test statistic will be closer to zero. Thus, the test is always a lower, one-tailed test regardless of which test statistic issues, Lk or Ek
...
Or for example, you may wonder "what is the likelihood that a person has an BMI of exactly___?
IKn this case, you would need to retrieve the density of the BMI distribution at values 140. The BMI distribution can be modeled with a mean of 100 and a standard deviation of 15. The corresponding density is:
...
You can see here, with a small sample size, we would reject the null hypothesis the samples cam from a normal distribution. We can increase the power of the test by increasing the sample size.
...
Lets create a function using the grubb test to identify all outliers. The grubbs test identifies outliers in a univariance dataset that is presumed to come from a normal distribution.
...
grubbs.flag <- function(x) {
  ...
  # as we identify them
  outliers <- NULL
  # we'll create a variable called test to identify which univariance we are testing
  test <- x
...
So far we've only looked at single words, but many interesting (more accurate) analyses are based on the relationship between wirds. 
...
```
This data is still in tidyrext format, and is structured as one-token-per-row. Each token is a bigram.

Counting amd filtering n-grams
...
We also may be interested in trigrams, which are three word combos
...
not_words %>%
  ...
  head(20) %>%
  ggplot(aes(x = contribution, y = reorder(word2, contribution), fill = contribution > 0)) +
  geom_col(show.legend = FALSE) +
...
```{r bigramgraph, dependson = "bigram_graph", fig.width = 12, fig.height = 10}
set.seed(1234)
